...
with engine.connect() as conn:
    # Query h_currencies
    result = conn.execute(text("SELECT symbol, date, rate FROM h_currencies_exh WHERE date > '2001-01-01'"))
    df = pd.DataFrame(result.fetchall(), columns=result.keys())

    # Query d_countries
    result2 = conn.execute(text("SELECT cod_currency, currency + ' (' + symbol + ')' as currency, cod_iso3, country FROM d_country WHERE cod_iso3 <> 'ATA'"))
    df_countries = pd.DataFrame(result2.fetchall(), columns=result2.keys())

    # Adjust SQS (Somaliland)
    sos_lines = [
        {"cod_currency": "SQS", "currency": "Somaliland Shilling (S)", "cod_iso3": 'SOS', "country": "Somaliland"},
    ]
...
# ============================================
# Map
url = 'https://raw.githubusercontent.com/guillemmaya92/Analytics/refs/heads/master/Maps/world-countries-sans-antarctica-all.json'
