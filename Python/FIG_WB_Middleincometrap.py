...
for color in df['color_dot'].unique():
    ...
    region_name = df_color['region'].iloc[0]  # Tomamos la región del primer elemento del grupo
    
    fig.add_trace(go.Scatter(
        x=df_color['start'],
        y=df_color['end'],
        mode='markers+text',
        name=region_name,  # Aquí ponemos la región, que será el texto en la leyenda
        marker=dict(
            size=df_color['pop_end'],
            color=color,
            line=dict(color=df_color['color_line'].iloc[0], width=2),
            sizemode='area',
            sizeref=1.5 * max(df['pop_end']) / (40. ** 2),
            sizemin=3
        ),
...
fig.update_layout(
    annotations=[
        ...
        dict(
            text=f"<b>Currency:</b> Official exchange rate 2023 of the local currency to USD.",
            xref="paper",
            yref="paper",
            x=0,
            y=-0.18,
...
filename = os.path.join(download_folder, f"FIG_WB_Middleincometrap")
