...
min_value = df['gdpcdl'].min()
max_value = df['gdpcdl'].max()
mean_value = df['gdpcdl'].median()
mean_value_r = df['gdpcd'].median() // 365
population = len(df)
year = df['date'].min()
...
sns.kdeplot(data=df, x="gdpcdl", hue="Region", bw_adjust=2.5, hue_order=order_region, multiple="stack", alpha=1, palette=custom_area, fill=True, linewidth=1, linestyle='-', ax=ax)
sns.kdeplot(data=df, x="gdpcdl", hue="Region", bw_adjust=2.5, hue_order=order_region, multiple="stack", alpha=1, palette=custom_line, fill=False, linewidth=1, linestyle='-', ax=ax)
...
plt.text(0, 1.02, 'Evolution by region from 1980 to 2030', fontsize=9, color='#262626', ha='left', transform=plt.gca().transAxes)
