...
df = df[['Region', 'Country_Abr', 'cod_country', 'pop_start', 'pop_end', 'gdpc_start', 'gdpc_end', 'gdpc_usa_rel_start', 'gdpc_usa_rel_end', 'start', 'end']]
...
# Median vs USA values
median_start = (df['gdpc_start'] * df['pop_start']).sum() / df['pop_start'].sum()
median_end = (df['gdpc_end'] * df['pop_end']).sum() / df['pop_end'].sum()

usa_start = df.loc[df['cod_country'] == 'USA', 'gdpc_start'].iloc[0]
usa_end = df.loc[df['cod_country'] == 'USA', 'gdpc_end'].iloc[0]

percent_start = median_start / usa_start * 100
percent_end = median_end / usa_end * 100
...
fig.add_shape(
    type="line",
    x0=math.log(percent_start),
    x1=math.log(percent_start),
    y0=0,
    y1=math.log(percent_end),
