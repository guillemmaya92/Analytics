...
# Filter after 2024
df = df[df['year'] == 2024]
df = df.dropna(subset=['LP', 'NGDPD', 'NGDPDPC'])
...
df = df[['iso3', 'Country', 'year', 'LP', 'NGDPD', 'NGDPDPC']]
...
filename = os.path.join(download_folder, f"FIG_IMG_GAP_GDPC_Population_{max_year}.png")
