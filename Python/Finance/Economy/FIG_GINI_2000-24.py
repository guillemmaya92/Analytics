...
# DataFrame Between-Country 2
# =====================================================================
# Copy Dataframe
dfb2 = df.copy()

# Sorting and formatting
dfb2 = dfb2[dfb2['Year'] == 2000]
dfb2 = dfb2.sort_values(by=['Year', 'NGDPDPC_Dis'])

# Calculating cummulative population and distribution
dfb2['GDP_Cum'] = dfb2.groupby('Year')['NGDPDPC_Dis'].cumsum() / dfb2.groupby('Year')['NGDPDPC_Dis'].transform('sum')
dfb2['POP'] = range(1, len(dfb2) + 1)
dfb2['POP_Cum'] = dfb2['POP'] / len(dfb2)

# World Mean and Median
dfb2['Mean']  = dfb2['NGDPDPC_Dis'].mean()
dfb2['Median']  = dfb2['NGDPDPC_Dis'].median()

# Calculation Gini
dfb2['Gini'] = dfb2.groupby('Year')['NGDPDPC_Dis'].transform(lambda x: gini(x))

# Selecting columns and filtering year
dfb2 = dfb2[['ISO3', 'Country', 'Year', 'Gini', 'GDP_Cum', 'POP_Cum', 'NGDPDPC_Dis', 'Mean', 'Median']]
...
dfmean2 = dfb2.loc[[ (dfb2['NGDPDPC_Dis'] - dfb2['Mean']).abs().idxmin() ]]
...
plt.plot(dfb2['POP_Cum'], dfb2['GDP_Cum'], label='Gini Between-Countries', color='darkred', linewidth=0.5, linestyle='-')
...
plt.text(x=xpop-0.02, y=ypop+0.05, 
        s=f'Median Population:\n{vpop: ,.0f}$\nBottom {(xpop) * 100: ,.0f}% earns {(ypop) * 100: ,.1f}%', 
        color='darkred', 
        va='center', 
        ha='center', 
        fontsize=8)
...
plt.text(x=xpop-0.09, y=ypop, 
         s=f'Median Income:\n{vpop: ,.0f}$\nTop {(1-xpop) * 100: ,.1f}% earns {(ypop) * 100: ,.0f}%',
         color='darkred', 
         va='center', 
         ha='center', 
         fontsize=8)
...
plt.text(x=xpop-0.06, y=ypop+0.05, 
        s=f'Mean Income:\n{vpop: ,.0f}$\nBottom{(xpop) * 100: ,.0f}% earns {(ypop) * 100: ,.0f}%',  
        color='dimgray', 
        va='center', 
        ha='center', 
        fontsize=8)

# Add scatter mean income 2000
xpop2 = dfmean2['POP_Cum'].iloc[0]
ypop2 = dfmean2['GDP_Cum'].iloc[0]
vpop2 = dfmean2['NGDPDPC_Dis'].iloc[0]
plt.scatter(x=xpop2, y=ypop2, color='dimgray', label='Mean Income', zorder=5, marker='o', facecolor='none')

# Add arrow from 2000 to 2024
plt.annotate('', xy=(xpop, ypop), xytext=(xpop2, ypop2),
             arrowprops=dict(facecolor='dimgray', edgecolor='dimgray', shrink=0.15, width=0.5, headwidth=4))

# Title and labels
plt.suptitle('   Global Income Inequality', fontsize=16, fontweight='bold', y=0.95)
plt.title('Evolution of the Gini Index from 2000 to 2024', fontsize=12, fontweight='bold', color='darkgrey', pad=20)
...
Ginib2 = dfb2['Gini'].iloc[-1] 

# Add legend
plt.text(0.05, 0.96, f'Gini Between-Countries (2024): {Ginib:.2f}', color='darkred', fontsize=9, fontweight='bold')
plt.text(0.05, 0.93, f'Gini Between-Countries (2000): {Ginib2:.2f}', color='darkred', fontsize=9)
plt.text(0.05, 0.90, f'Gini Within-Countries: {Giniw:.2f}', color='darkblue', fontsize=9)
plt.text(0.05, 0.87, 'Perfect Distribution: 0', color='darkgrey', fontsize=9, fontweight='bold')
...
# Save the figure
plt.savefig('C:/Users/guillem.maya/Desktop/FIG_GINI_2024.png', format='png', dpi=300, bbox_inches='tight')
